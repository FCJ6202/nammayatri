  ...
  case state.data.driverInfoResponse of
    Just (GetDriverInfoResp res) ->
      let
        ...
        blockedTitle = if isSuspended then "Suspended" else "Blocked"
        driverBlockedHeaderVisibility = true --state.data.driverBlocked
      ...
      in
        linearLayout
          ...
          [ textView $
              ...
              , text blockedTitle -- $ getString $ BLOCKED_TILL (EHC.convertUTCtoISC state.data.blockedExpiryTime "hh:mm A") (EHC.convertUTCtoISC state.data.blockedExpiryTime "DD-MM-YYYY")
              , weight 1.0
    ...
    Just (GetDriverInfoResp resp) ->
       case resp.overchargingTag of
        Just overchargingTag ->
          ...
          , ExtraChargeCard.view (push <<< ExtraChargeCardAC) $ ExtraChargeCard.extraChargesConfig overchargingTag resp.ridesWithFareIssues resp.totalRidesConsideredForFareIssues
          ]
        Nothing -> linearLayout [width $ V 0, height $ V 0][]
    Nothing -> linearLayout [width $ V 0, height $ V 0][]
