...
frfsBookingStatus (personId, merchantId_) booking' = do
  ...
  integratedBPPConfigId <- booking.integratedBppConfigId & fromMaybeM (InternalError "integratedBppConfigId not found")
  ...
  case booking.status of
    ...
    DFRFSTicketBooking.CONFIRMED -> do
      void $ CallExternalBPP.status merchant.id merchantOperatingCity bapConfig booking integratedBppConfig.platformType
      buildFRFSTicketBookingStatusAPIRes booking paymentSuccess
            ...
            then do
              ...
              if paymentBookingStatus == FRFSTicketService.SUCCESS
                then do
                  ...
                  void $ CallExternalBPP.confirm (processOnConfirm integratedBppConfig) merchant merchantOperatingCity bapConfig (mRiderName, mRiderNumber) updatedBooking integratedBppConfig.platformType
  ...
  where
    ...
    processOnConfirm integratedBppConfig onConfirmReq = do
      (merchant', booking'') <- DACFOC.validateRequest onConfirmReq integratedBppConfig.platformType
...
buildFRFSTicketBookingStatusAPIRes booking payment = do
  integratedBppConfigID <- booking.integratedBppConfigId & fromMaybeM (InternalError "integratedBppConfigId not found")
  integratedBppConfig <- QIBP.findById integratedBppConfigID >>= fromMaybeM (InvalidRequest "integratedBppConfigID not found in Booking")
  stations <- mapM (Utils.mkPOrgStationAPI booking.partnerOrgId booking.merchantOperatingCityId booking.vehicleType integratedBppConfig.platformType) =<< (decodeFromText booking.stationsJson & fromMaybeM (InternalError "Invalid stations jsons from db"))
